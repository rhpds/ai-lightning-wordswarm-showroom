...
Reasoning models like qwen36-27b support extended thinking -- an internal chain-of-thought that runs before the model produces visible output. When served via vLLM, you can toggle thinking on or off using the `chat_template_kwargs` parameter.
...
When thinking is enabled (the default for qwen36-27b), the model performs internal chain-of-thought reasoning before producing its final answer. The reasoning tokens appear in the `reasoning` field of the response message.
...
curl -s -H "Authorization: Bearer $TOKEN" \
  https://maas.apps.ocp.cloud.rhai-tmm.dev/prelude-maas/qwen36-27b/v1/models | jq .
----
+
...
[source,bash]
----
curl -s https://maas.apps.ocp.cloud.rhai-tmm.dev/prelude-maas/qwen36-27b/v1/chat/completions \
...
[source,bash]
----
time curl -s https://maas.apps.ocp.cloud.rhai-tmm.dev/prelude-maas/qwen36-27b/v1/chat/completions \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "qwen36-27b",
...
----
+
Compare the response quality with qwen36-27b's reasoning-enabled response. Notice there is no `reasoning` field.
...
. Select **qwen36-27b** from the model dropdown (it's a reasoning model)

. Click **START AGENT** and watch the agent play
+
image::agent-running.png[Agent running with qwen36-27b,link=self,window=blank,width=700]

. Observe the stats panel while qwen36-27b plays:
...
✓ Ran agent with both qwen36-27b (reasoning) and Llama 3.2 3B (non-reasoning) +
...
* qwen36-27b uses `"chat_template_kwargs": {"enable_thinking": false}` to disable reasoning (not prompt tags)
