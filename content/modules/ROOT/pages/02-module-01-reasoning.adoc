...
Reasoning models like nemotron-cascade-2-30b support extended thinking -- an internal chain-of-thought that runs before the model produces visible output. When served via vLLM, you can toggle thinking on or off using the `chat_template_kwargs` parameter.
...
When thinking is enabled (the default for nemotron-cascade-2-30b), the model performs internal chain-of-thought reasoning before producing its final answer. The reasoning tokens appear in the `reasoning` field of the response message.

* **Thinking ON** (default) -- The model reasons step-by-step internally. Higher quality but more tokens and latency.
* **Thinking OFF** -- Pass `"chat_template_kwargs": {"enable_thinking": false}` to skip reasoning. Faster and cheaper, but may miss nuance.
...
curl -s -H "Authorization: Bearer $TOKEN" \
  https://maas.apps.ocp.cloud.rhai-tmm.dev/prelude-maas/nemotron-cascade-2-30b/v1/models | jq .
----
+
...
[source,bash]
----
curl -s https://maas.apps.ocp.cloud.rhai-tmm.dev/prelude-maas/nemotron-cascade-2-30b/v1/chat/completions \
...
[source,bash]
----
time curl -s https://maas.apps.ocp.cloud.rhai-tmm.dev/prelude-maas/nemotron-cascade-2-30b/v1/chat/completions \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "nemotron-cascade-2-30b",
...
----
+
Compare the response quality with nemotron-cascade-2-30b's reasoning-enabled response. Notice there is no `reasoning` field.
...
. Select **nemotron-cascade-2-30b** from the model dropdown (it's a reasoning model)

. Click **START AGENT** and watch the agent play
+
image::agent-running.png[Agent running with nemotron-cascade-2-30b,link=self,window=blank,width=700]

. Observe the stats panel while nemotron-cascade-2-30b plays:
...
✓ Ran agent with both nemotron-cascade-2-30b (reasoning) and Llama 3.2 3B (non-reasoning) +
...
* nemotron-cascade-2-30b uses `"chat_template_kwargs": {"enable_thinking": false}` to disable reasoning (not prompt tags)
