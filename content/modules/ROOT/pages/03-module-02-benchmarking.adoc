...
== Exercise 2: Benchmark qwen36-27b (reasoning model)

qwen36-27b is the top-performing model on the WordSwarm leaderboard. Let's measure its raw inference performance.

NOTE: qwen36-27b is a reasoning model with thinking enabled by default. When thinking is on, the model buffers its internal chain-of-thought before flushing all output tokens in a single chunk -- TTFT and ITL will show as 0.0ms because GuideLLM can't measure per-token timing. To get real TTFT/ITL metrics, we disable thinking via the `extras` backend kwarg below.
...
----
guidellm benchmark \
  --target "https://maas.apps.ocp.cloud.rhai-tmm.dev/prelude-maas/qwen36-27b/v1" \
  ...
  --max-seconds 30 \
  --processor "gpt2" \
  --output-dir ./results/qwen36-27b/concurrent-10 \
...
✓ Benchmark completes in ~30 seconds +
✓ You can compare the numbers with qwen36-27b's results
...
**qwen36-27b is faster across every metric** despite being a massive reasoning model!

=== Why does this happen?

**GPU allocation matters.** qwen36-27b runs on 8x H200 GPUs with massive memory bandwidth and compute. Llama 3.2 3B runs on a single tiny MIG slice with limited resources.
...
qwen36-27b generates tokens faster, finds better words, and scores ~29x higher despite lower accuracy. Quality matters more than speed for complex reasoning tasks.
